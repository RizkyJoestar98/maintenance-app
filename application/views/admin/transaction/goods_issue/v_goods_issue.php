    ...
    <section class="content">
        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title">
                    <a href="<?= site_url('admin/add_goods_issue') ?>" class="btn btn-primary"><i
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <button type="button" class="btn btn-info" data-toggle="modal"
                                    data-target="#updategi<?= $value->id_transaction; ?>">
                                    <i class="fas fa-edit mr-2"></i>Update
                                </button>
                                <button type="button" class="btn btn-danger" id="delete_gi"
                                    data-id-transaction="<?= $value->id_transaction; ?>"
                                    data-transaction-type="<?= $value->transaction_type; ?>"><i
                                        class="fas fa-trash mr-2"></i>Delete</button>
                            </td>
                        </tr>

                        <!-- Modal Edit Goods Issue -->

                        <div class="modal fade" id="updategi<?= $value->id_transaction; ?>" tabindex="-1"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <?= form_open('admin/update_goods_issue', array('id' => 'form_update_goods_issue_' . $value->id_transaction)); ?>
                                    <div class="card-body">
                                        <label for="id_transaction">ID Transaction</label>
                                        <div class="form-group">
                                            <input type="text" name="id_transaction" class="form-control"
                                                id="id_transaction<?= $value->id_transaction; ?>"
                                                value="<?= $value->id_transaction; ?>">
                                        </div>
                                        <label for="transaction_type">Transaction Type</label>
                                        <div class="form-group">
                                            <input type="text" name="transaction_type" class="form-control"
                                                id="transaction_type<?= $value->id_transaction; ?>"
                                                value="<?= $value->transaction_type; ?>">
                                        </div>
                                        <label for="date_transaction">Date Transaction</label>
                                        <div class="form-group">
                                            <input type="datetime-local" name="date_transaction" class="form-control"
                                                id="date_transaction<?= $value->id_transaction; ?>"
                                                value="<?= $value->date; ?>">
                                        </div>
                                        <label for="code_material">Material Code</label>
                                        <div class="form-group">
                                            <input type="text" name="code_material" class="form-control"
                                                id="code_material<?= $value->id_transaction; ?>"
                                                value="<?= $value->code_material; ?>">
                                        </div>
                                        <label for="quantity">Quantity</label>
                                        <div class="form-group">
                                            <input type="number" name="quantity" class="form-control"
                                                id="quantity<?= $value->id_transaction; ?>"
                                                value="<?= $value->quantity; ?>" min="0">
                                        </div>
                                        <label for="identity_pic">Identity PIC</label>
                                        <div class="form-group">
                                            <input type="text" name="identity_pic" class="form-control"
                                                id="identity_pic<?= $value->id_transaction; ?>"
                                                value="<?= $value->identity_pic; ?>">
                                        </div>
                                        <label for="description">Description</label>
                                        <div class="form-group">
                                            <input type="text" name="description" class="form-control"
                                                id="description<?= $value->id_transaction; ?>"
