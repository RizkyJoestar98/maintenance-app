                    ...
                    <div class="modal fade" id="upload_excel_machine" data-backdrop="static" data-keyboard="false"
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <?= form_open_multipart('admin/upload_machine', array('id' => 'form_upload_excel_machine')) ?>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <button type="button" class="btn btn-info" data-toggle="modal"
                                    data-target="#update_machine_<?= $value->id_machine; ?>">
...
<div class="modal fade" id="update_machine_<?= $value->id_machine; ?>" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <?= form_open('admin/update_machine', array('id' => 'form_update_machine_' . $value->id_machine)); ?>
            <div class="card-body">
                <input type="hidden" class="form-control" id="id_machine<?= $value->id_machine ?>" name="id_machine"
                    value="<?= $value->id_machine ?>" readonly>
                <div class="form-group">
                    <label>Area</label>
                    <select class="form-control select2" id="area<?= $value->id_machine ?>" name="area"
                        style="width: 100%;">
                        <option selected="selected" value="">- Select Area -</option>
                    </select>
                </div>
                <input type="hidden" class="form-control" id="code_area_now<?= $value->id_machine ?>"
                    name="code_area_now" value="<?= $value->code_area ?>" readonly>
                <div class="form-group">
                    <label for="">Area Now</label>
                    <input type="text" class="form-control" id="area_now<?= $value->id_machine ?>" name="area_now"
                        value="<?= $value->name_area ?>" readonly>
                </div>
                <div class="form-group">
                    <label>Line</label>
                    <select class="form-control select2" id="line<?= $value->id_machine ?>" name="line"
                        style="width: 100%;">
                        <option selected="selected" value="">- Select Line -</option>
                    </select>
                </div>
                <input type="hidden" class="form-control" id="code_line_now<?= $value->id_machine ?>"
                    name="code_line_now" value="<?= $value->code_line ?>" readonly required>
                <div class="form-group">
                    <label for="">Line Now</label>
                    <input type="text" class="form-control" id="line_now<?= $value->id_machine ?>" name="line_now"
                        value="<?= $value->name_line ?>" readonly required>
                </div>
                <div class="form-group">
                    <label for="code_machine">Code Machine</label>
                    <input type="text" class="form-control" id="code_machine<?= $value->id_machine ?>"
                        name="code_machine" placeholder="Enter Code Machine" value="<?= $value->code_machine ?>">
                </div>
                <div class="form-group">
                    <label for="name_machine">Name Machine</label>
                    <input type="text" class="form-control" id="name_machine<?= $value->id_machine ?>"
                        name="name_machine" placeholder="Enter Name Machine" value="<?= $value->name_machine ?>">
                </div>
            </div>
            <!-- /.card-body -->
...
$(document).ready(function() {
    ...
    $('#form_upload_excel_machine').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= base_url('admin/upload_machine') ?>',
            data: formData,
            dataType: 'JSON',
            contentType: false,
            processData: false,
    ...
    $(document).on('click', 'button[data-id-machine]', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: '<?= site_url('admin/delete_machine') ?>',
                    data: {
